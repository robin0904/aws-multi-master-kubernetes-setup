#!/usr/bin/env bash
# =============================================================================
# worker-userdata.sh.tpl  —  Worker node full bootstrap
#
# Injected template variables:
#   hostname      — e.g. dev-k8s-cluster-worker-1
...
echo "=== worker-${node_index} user-data started: $(date) ==="
...
echo "NODE_ROLE=worker"           >> /etc/environment
...
# === Phase 8 — Poll SSM for worker join command ===
echo "Waiting for master-1 READY + worker join command..."
RETRIES=0
until [[ "$(ssm_get "$${SSM_PATH}/bootstrap/master1_init_status")" == "READY" ]]; do
  RETRIES=$((RETRIES+1))
  [[ "$${RETRIES}" -ge 60 ]] && { echo "ERROR: master-1 never ready"; exit 1; }
  echo "Waiting for master-1... ($${RETRIES}/60)"
  sleep 15
done

JOIN_CMD=$(ssm_get "$${SSM_PATH}/bootstrap/worker_join_cmd")
if [[ -z "$${JOIN_CMD}" || "$${JOIN_CMD}" == "PENDING" ]]; then
  echo "ERROR: worker_join_cmd is empty"
  exit 1
fi

echo "Running worker join..."
eval "$${JOIN_CMD} --node-name ${hostname}" 2>&1 | tee /tmp/kubeadm-join.log

sleep 5
if systemctl is-active --quiet kubelet; then
  echo "kubelet is active — join successful"
else
  echo "ERROR: kubelet not active after join"
  systemctl status kubelet --no-pager || true
  exit 1
fi

echo "=== worker-${node_index} bootstrap COMPLETE: $(date) ==="
